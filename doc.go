...
deleted. Each cache instance has a maximum size, defined in the number of used bytes. If a new item doesn't fit
...
total allocated memory into segments. One segment can contain data only from one item. One item can span over 0
or any number of segments (0 when both the key and the content have 0 length). The segment size is defined in
the accordingly named option.

The segmentation means that Forget can only store maximum MaxSize / SegmentSize number of items. It also means
that, since the items of arbitrary size, the total allocated memory will probably be never fully utilized. The
utilization can be designed for the expected usage pattern and controlled by selecting the best fitting segment
size. Typically, it is better to choose a segment size a 'couple of times' smaller than the expected size of the
'bulk' of the items, but choosing 'too small' segment size can negatively impact the retrieve time and the
required metadata can take too much space besides what's defined in MaxSize. The default settings segment size
